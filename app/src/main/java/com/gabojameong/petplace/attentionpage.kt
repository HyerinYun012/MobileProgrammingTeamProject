...
class AttentionPage : AppCompatActivity() {
    ...
    private fun fetchFavoriteListFromServer() {
        ...
        apiService.getBookmarks(pageable).enqueue(object : Callback<ApiResponse<PageResponse<BookmarkResponse>>> {
            ...
            override fun onFailure(call: Call<ApiResponse<PageResponse<BookmarkResponse>>>, t: Throwable) {
                Log.e("API_ERROR", "통신 실패: ${t.message}")
        ...
        adapter = FavoriteShopAdapter(dataList) { restaurantId, _ ->
            ...
            apiService.toggleBookmark(restaurantId).enqueue(object : Callback<ApiResponse<Boolean>> {
                override fun onResponse(call: Call<ApiResponse<Boolean>>, response: Response<ApiResponse<Boolean>>) {
                    if (response.isSuccessful && response.body()?.success == true) {
                        Log.d("API_SUCCESS", "$restaurantId 번 업장 찜 상태 변경 성공!")
                    } else {
