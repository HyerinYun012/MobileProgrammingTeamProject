...
import com.example.petplace.databinding.ActivityFindPwBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnFindPw.setOnClickListener {
            ...
            if (loginId.isEmpty() || phone.isEmpty()) {
                Toast.makeText(this, "아이디와 전화번호를 입력해주세요.", Toast.LENGTH_SHORT).show()
            ...
            if (newPassword != newPassword2) {
                Toast.makeText(this, "새비밀번호가 새비밀번호 확인과 일치하지 않습니다.", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            ...
            apiService.resetPassword(request).enqueue(object : Callback<ApiResponse<Any>> {
                override fun onResponse(call: Call<ApiResponse<Any>>, response: Response<ApiResponse<Any>>) {
                    ...
                    if (response.isSuccessful && apiResponse?.success == true) {
                        Toast.makeText(this@FindPwActivity, "비밀번호가 안전하게 변경되었습니다.\n새 비밀번호: $newPassword", Toast.LENGTH_LONG).show()
                        finish()
                    } else {
                        val errorMsg = apiResponse?.message ?: "정보가 일치하지 않거나 요청에 실패했습니다."
                        Toast.makeText(this@FindPwActivity, errorMsg, Toast.LENGTH_SHORT).show()
                    }
                }
